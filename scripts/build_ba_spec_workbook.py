#!/usr/bin/env python3
"""Generate WEREAL REOS BA spec workbook — quản lý màn hình, UC, test & traceability."""
...
OUTPUT = ROOT / "docs" / "samples" / "WEREAL_BA_Spec.xlsx"

SCR_ID_RE = re.compile(r"SCR-[A-Z0-9]+-\d+")
UC_ID_RE = re.compile(r"UC-[A-Z]+-\d+")

from wereal_ba_catalog_data import (  # noqa: E402
    BUSINESS_RULES,
    CODE_REGISTRY,
    MODULES,
    SCREENS,
...
from wereal_ba_catalog_data import MODULE_TAB  # noqa: E402
...
def _write_uc_detail_sheet(
    ...
    *,
    ...
    badge = "Spec thủ công P0" if detail.get("_manual") else "Spec auto"
...
def _build_tong_quan(wb: Workbook, scr_sheet_map: dict[str, str], uc_sheet_map: dict[str, str]) -> None:
    ...
    ws["A1"] = "WEREAL REOS — Bộ tài liệu BA & Quản lý dự án"
    ws["A1"].font = _font(bold=True, size=18, color=NAVY)
    ws["A1"].alignment = Alignment(vertical="center")
    ws.row_dimensions[1].height = 36

    ws.merge_cells("A2:F2")
    ws["A2"] = f"Real Estate Operating System (PropTech SaaS) · Phiên bản {VERSION} · Xuất {TODAY}"
    ...
    info = [
        ...
        ("Spec master", "docs/specs/WEREAL-BA-Master-Spec.md"),
        ("SRS / UC doc", "Tai-lieu-yeu-cau-phan-mem.md · Danh-sach-use-case-user-story.md"),
        ("Prototype", "prototype/ — React v2 Premium · 78 UC screens"),
        ("API contract", "openapi.yaml — 71 endpoints (NestJS BFF)"),
        ...
        ("Quy ước mã", "SCR / UC / TC / BR / FR / CON / ADR / BL"),
    ]
    r = 4
...
def _build_01_danh_sach_man_hinh(
    ...
    headers = [
        ...
        "FR / Trace", "Last Updated", "Ghi chú triển khai", "→ Sheet spec",
    ]
...
def _build_04_index_use_case(wb: Workbook, uc_sheet_map: dict[str, str]) -> None:
    ...
    for uc_id in sorted(uc_sheet_map):
        row = uc_by_id[uc_id]
        prefix = uc_id.split("-")[1] if uc_id.startswith("UC-") else uc_id.split("-")[0]
...
def _build_07_change_log(wb: Workbook) -> None:
    ...
    rows = [
        [TODAY, VERSION, "Deep-spec P0 batch-3: COM(5) + TR(4) UC + 9 SCR (commission, audit, dispute)", "BA Team", "wereal_ba_uc_details_com|tr.py"],
        ["2026-07-28", "1.2", f"Deep-spec P0 batch-2: CRM(3) + LS(5) + ID(4) UC + 11 SCR (+ SCR-ADMIN-016 enhanced)", "BA Team", "wereal_ba_uc_details_crm|ls|id.py"],
        ["2026-07-28", "1.1", "Deep-spec P0 batch-1: GR(5) + BK(4) + PAY(3) UC + 12 SCR", "BA Team", "wereal_ba_uc_details_gr|bk|pay.py"],
        ["2026-07-28", "1.0", f"Khởi tạo workbook — {len(SCREENS)} SCR, {len(USE_CASES)} UC auto", "BA Team", "wereal_ba_catalog_data.py"],
    ]
...
def main() -> None:
    ...
    wb.save(OUTPUT)
    print(f"Wrote {OUTPUT} ({len(wb.sheetnames)} sheets, v{VERSION})")
